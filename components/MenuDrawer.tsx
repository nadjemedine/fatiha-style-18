...
const MenuDrawer = () => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex">
      ...
      {/* Drawer */}
      <div className="relative mr-auto h-full w-[80%] sm:w-1/2 bg-[#f7faf7] shadow-2xl flex flex-col animate-slide-right border-r border-[#ccd9cc]">
        <div className="p-4 border-b border-[#ccd9cc] bg-[#e2eee2]">
          ...
          {/* Tabs */}
          <div className="flex bg-[#ccd9cc]/30 p-1 rounded-2xl">
            <button 
              onClick={() => setActiveTab('boutique')}
              className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl font-bold text-sm transition-all ${
                activeTab === 'boutique' ? 'bg-white text-[#2d5a27] shadow-sm' : 'text-gray-500'
              }`}
            >
            ...
            <button 
              ...
              className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl font-bold text-sm transition-all ${
                activeTab === 'pages' ? 'bg-white text-[#2d5a27] shadow-sm' : 'text-gray-500'
              }`}
            >
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {activeTab === 'boutique' ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center opacity-40">
              <Store className="w-12 h-12 mb-3 text-[#2d5a27]" strokeWidth={1} />
            ...
            <div className="space-y-2">
              {pages.map((page) => (
                <Link 
                  ...
                  href={page.path}
                  onClick={() => setIsMenuOpen(false)}
                  className="w-full flex items-center justify-between p-4 bg-white rounded-2xl border border-[#ccd9cc] group hover:bg-[#e2eee2] transition-colors"
                >
                  <span className="font-bold text-gray-700">{page.name}</span>
                  <ChevronRight className="w-5 h-5 text-gray-300 group-hover:text-[#2d5a27] transition-colors" />
                </Link>
              ))}
            </div>
          )}
        ...
        <div className="p-6 border-t border-[#ccd9cc] bg-white">
